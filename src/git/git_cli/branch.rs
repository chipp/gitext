use git2::{BranchType, ErrorClass, ErrorCode, Remote, Repository};
...
pub fn switch_to_branch(
    ...
    match find_remote_branch(branch_name, &remote, &repo) {
        Ok(_) => switch_to_existing_branch(branch_name, repo),
...
fn switch_to_existing_branch(branch_name: &str, repo: &Repository) -> Result<(), Error> {
